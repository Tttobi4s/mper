...
dataspace_filter = int.from_bytes(
    sock4.recv((3 * cnt) // 8), 'big') ^ dataspace_filter
dataspace_filter = bin(dataspace_filter)[2:]
dataspace_filter = "0" * (3 * cnt - len(dataspace_filter)) + dataspace_filter
dataspace_filter_1 = []
for i in range(cnt):
    if dataspace_filter[i * 3 : i * 3 + 3] == "000":
        dataspace_filter_1.append("0")        
    else:
        dataspace_filter_1.append("1")       
new_cnt = dataspace_filter_1.count("0")
print(new_cnt)
sock4.send(("%d" % new_cnt).encode('utf-8'))
dataspace_filter_1 = int("".join(dataspace_filter_1),base=2)
sock1.send(dataspace_filter_1.to_bytes(cnt // 8, 'big'))
sock2.send(dataspace_filter_1.to_bytes(cnt // 8, 'big'))
sock3.send(dataspace_filter_1.to_bytes(cnt // 8, 'big'))
